            ...
            let classify = |reloc: &R, sym_option: Option<&Elf32_Sym>| -> Option<RelocInfo<R>> {
                ...
                match self.isa {
                    ...
                    Isa::RiscV32 => match reloc.type_info() {
                        // R_RISCV_CALL_PLT spans an AUIPC+JALR pair (8 bytes): the upper
                        // 20 bits of the value go into AUIPC at r_offset, the lower 12
                        // (signed) go into JALR at r_offset+4. Unlike PCREL_HI20, there
                        // is no companion LO12 reloc — this single entry must patch both.
                        R_RISCV_CALL_PLT => Some(RelocInfo {
                            defined_val,
                            indirect_reloc: None,
                            pc_relative: true,
                            relocate: Some(Box::new(|target_word, value| {
