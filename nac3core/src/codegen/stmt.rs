        ...
        Fn(&mut CodeGenContext<'ctx, '_>, IntValue<'ctx>) -> anyhow::Result<BasicValueEnum<'ctx>>,
{
    ...
    gen_for_callback(
        ...
        |_, ctx| {
            ...
            let iv_pair = ctx.build_allocate(
...
pub fn gen_try<'ctx, 'a, G: CodeGenerator>(
    ...
    if has_cleanup {
        let final_state = ctx.build_allocate(
            AllocationScope::StackStartOfFunc,
            ptr_type,
            Some("try.final_state.addr"),
        )?;
    ...
    for (handler_node, exn_type) in handlers.iter().zip(clauses.iter()) {
        ...
        if let Some(name) = name {
            ...
            let exn_store = ctx.build_allocate(
                AllocationScope::StackStartOfFunc,
                exn_ty,
                Some("try.exn_store.addr"),
            )?;
