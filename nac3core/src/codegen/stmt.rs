...
use super::{
    ...
    types::{EnumerateType, ExceptionType, ListType, RangeType, ndarray::NDArrayType},
    values::{
...
use crate::{
    codegen::{
        ...
        values::{TupleValue, UntypedArrayLikeAccessor},
    },
        ...
        |_, ctx, iv_pair| {
            ...
            if element_ty.is_some() {
                ...
                for (idx, (_, case_bb)) in cases.iter().take(length as usize).enumerate() {
                    ...
                    let elem_val = tuple.extract_element(ctx, idx as u32);
...
pub fn gen_for<G: CodeGenerator>(
    ...
    match &*ctx.unifier.get_ty(iter_ty) {
        ...
        {
            let enumerate = EnumerateType::new(ctx)
                .map_pointer_value(iter_val.into_pointer_value(), Some("enumerate"));
            let start = enumerate.load_start(ctx);
            let Some(target_i) =
                generator.gen_store_target(ctx, target, Some("for.target.addr"))?
            else {
                codegen_unreachable!(ctx)
            ...
            let (iterable_ty, iterable_val) = if let ExprKind::Call { args, .. } = &iter.node {
                ...
                let iterable_ptr = enumerate.ptr_to_iterable(ctx);

                let iterable_struct_ptr = ctx
                    .builder
                    .build_load(iterable_ptr, "iterable_ptr")
                    .unwrap()
                    .into_pointer_value();

                let iterable_struct_val = ctx
                    .builder
                    .build_load(iterable_struct_ptr, "iterable_struct")
                    .unwrap()
                    .into_struct_value();
                ...
                let iterable_ptr = ctx
                    .builder
                    .build_pointer_cast(
                        iterable_data_i8ptr.into_pointer_value(),
                        iterable_llvm_ty.ptr_type(inkwell::AddressSpace::default()),
                        "iterable_ptr",
                    )
                    .unwrap();
                let ag = ctx.builder.build_load(iterable_ptr, "iterable_struct").unwrap();
                (iterable_ty, ag)
            };

            match &*ctx.unifier.get_ty(iterable_ty) {
                TypeEnum::TTuple { ty: tuple_tys, .. } => {
                    let iterable = TupleValue::from_struct_value(
                        iterable_val.into_struct_value(),
                        ctx.size_t,
                        Some("tuple_iterable"),
                    );
